...
# Compute days since 2011-01-05 for init_time_days
# full_date="${year}-${month}-${day}" not working because year is 0054
# Reference date
# reference_date="2011-01-05"

# Convert both dates to seconds since epoch
# start_epoch=$(date -d "$reference_date" +%s)
# end_epoch=$(date -d "$full_date" +%s)
...
# new_value=$((149753 + days_diff))
# echo "init_time_days is ${new_value}"
#sed -i "s/^\( *init_time_days *= *\).*,/\1${new_value},/" input_filter.nml
}
